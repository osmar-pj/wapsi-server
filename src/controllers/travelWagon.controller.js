...
export const getTravelWagon = async (req, res) => {
    try {
        ...
        const graphic = (period === 'S' ? data.graphic.slice(0, 7) : data.graphic).map(i => {
            ...
            // if (NOCHE === 0 && DIA > 0) {
            //     resultObject = { DIA };
            // } else if (DIA === 0 && NOCHE > 0) {
            //     resultObject = { NOCHE };
            // } else if (DIA > 0 && NOCHE > 0) {
            //     resultObject = { NOCHE, DIA };
            // }

            // resultObject.day = i.day;
            // resultObject.date = i.date;
            // resultObject.x = x;
        ...
        const locomotives = data.locomProduction.map(i => {
            ...
            const turn = {
                DIA: { value: weightDia, percent: weightDia * 100 / totalWeight, color: colorDia, msg: `Dia/ ${weightDia}` },
                NOCHE: { value: weightNoche, percent: weightNoche * 100 / totalWeight, color: colorNoche, msg: `Noc/ ${weightNoche}` }
            }
            ...
            const material = {
                MINERAL: { value: weightMineral, percent: weightMineral * 100 / totalWeight, color: '#5993af', msg: `Min/ ${weightMineral}` },
                ...
                DESM: { value: weightDesm, percent: weightDesm * 100 / totalWeight, color: '#ff6d7a', msg: `Des/ ${weightDesm}`}
            }
